...
#Date: 2025-7-22
#CIS Benchmarks for SQL Server 2017 Version: 1.3.0



# PowerShell script to audit SQL Server 2017 against CIS Benchmarks v1.3.0 (Automated checks only)
...
# CIS Check 2.18: Ensure 'clr strict security' is enabled
$query = @"
SELECT name,
...
if ($dataset -and $null -ne $dataset.value_configured -and $null -ne $dataset.value_in_use) {
    ...
    Add-Result -CheckId "2.18" -Description $description -Status $status `
               -CurrentValue "value_configured = $valueConfigured, value_in_use = $valueInUse" -ExpectedValue "value_configured = 1, value_in_use = 1"
} else {
    Add-Result -CheckId "2.18" -Description "Ensure 'clr strict security' is enabled" `
